...
def remove_horizontal_lines(img):
    ...
    horizontal_structure = cv2.getStructuringElement(cv2.MORPH_RECT, (horiz_size, 1))

    # Preform erode function with structuring element in order to find the
    # image lines.
    horizontal = cv2.erode(src=horizontal, kernel=horizontal_structure, anchor=(-1, -1))

    # Preform dilate function in order to connect some gaps in found lines.
    horizontal = cv2.dilate(src=horizontal, kernel=horizontal_structure, anchor=(-1, -1))
